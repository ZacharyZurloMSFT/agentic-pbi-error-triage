...
Single-page live dashboard for the the customer BI Triage demo. Replaces the
...
Thin FastAPI proxy over `func-triage`. All data reads (SQL + Graph) happen
inside the Function App using its managed identity; the cockpit just
forwards requests with an `api://func-triage` bearer token.
...
FastAPI cockpit (this app)
  ...
  | Authorization: Bearer <api://func-triage token via az login>
  v
func-triage (Easy Auth v2 gate: presenter OID + Foundry MI in allowedPrincipals)
...
3. **The presenter's Entra object id must be in `func-triage`'s Easy Auth
   ...
   ```
4. `dbo.demo_events` table exists — created automatically by `func-triage`'s
...
| `GET  /api/inbox`   | last 5 `[BI-DEMO]` messages (via `func-triage`)             |
...
Under the hood these forward to `func-triage`'s `/api/demo/{inbox,events,rows+flags,cleanup}`
...
- **401 from `/api/*`** - your Entra OID isn't in `func-triage`'s
  `allowedPrincipals`. See prerequisite 3.
- **`Cannot acquire api://func-triage token`** - `az login` hasn't happened, or
...
- **Adaptive Card renders as plain text in the Teams tile** - `func-triage`
